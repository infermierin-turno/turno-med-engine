...
app = FastAPI(title="TurnoMed Python Engine", version="2.2.0")
...
async def genera_turni(data: GenerazioneRequest):
    ...
    async with httpx.AsyncClient() as client:
        try:
            # 1. Recupero degli operatori (proviamo prima 'utenti', se fallisce prova 'staging_utenti')
            ...
            if resp_utenti.status_code != 200:
                # Tentativo di fallback sulla tabella staging_utenti se 'utenti' restituisce errore
                ...
                if resp_utenti.status_code != 200:
                    raise HTTPException(status_code=500, detail=f"Errore Supabase utenti: {resp_utenti.text}")
            
            operatori = resp_utenti.json()
            if not operatori:
                raise HTTPException(status_code=400, detail=f"Nessun operatore trovato per organizzazione {data.organizzazione_id} e reparto {data.reparto_id}.")
